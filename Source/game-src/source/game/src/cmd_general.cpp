...
ACMD(do_restart)
{
	...
	if (subcmd != SCMD_RESTART_TOWN && (!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG))
	{
		if (!test_server)
		{
			...
			if (iTimeToDead > 170)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("No new start possible.(%d seconds left) "), iTimeToDead - 170);
				return;
			}
		}
	}
	...
	if (subcmd == SCMD_RESTART_TOWN)
	{
		...
		if (iTimeToDead > 173)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Still no new start possible in the city. (%d seconds left)"), iTimeToDead - 173);
			return;
		}
	}
