...
std::string GetLowerName(std::string Filename)
{
...
void EterIndex::PutFile(const char* szFilename, enum STORAGE_TYPE eType, UINT32 iFileSize, UINT32 iHashCRC32)
{
    ...
    auto lowerName = GetLowerName(szFilename);

    printf("Adding: %s\n", lowerName.c_str());

    hEterIndexItem.Index = this->iIndex;
    hEterIndexItem.UnkZeroDword = 0;
    hEterIndexItem.VirtualPathCRC32 = GetCRC32(lowerName.c_str(), lowerName.size());
    ...
    strncpy(hEterIndexItem.VirtualPath, &lowerName[0], 160); //<-- 160 is i sus wrong
