	...
	if app.ENABLE_TARGET_INFO:
		class InfoBoard(ui.ThinBoard):
			...
			def __init__(self):
				...
				wndItem.ArrangeSlot(0, self.X_SLOT, self.Y_SLOT, 32, 32, 0, 0)
				wndItem.RefreshSlot()
				...
				self.itemArray = [0 for i in xrange(self.TARGET_ARRAY)]
				self.itemVnums = [0 for i in xrange(self.TARGET_ARRAY)]
			...
			def __LoadInformation(self, race):
				...
				for i in xrange(self.TARGET_ARRAY):
			...
			def SearchEmptySlot(self, size):
				for value in xrange(self.TARGET_ARRAY):
					if 0 == self.itemArray[value]:
						...
						for i in range(size - 1):
							emptySlotIndex = emptySlotIndex + self.X_SLOT

							if emptySlotIndex >= self.TARGET_ARRAY:
								searchSucceed = False
								continue
			...
			def __LoadInformation_Drops(self, race):
				...
				if race in MONSTER_INFO_DATA:
					if len(MONSTER_INFO_DATA[race]["items"]) == 0:
						...
						self.wndItem.SetPosition(13, self.yPos - 4)
						self.yPos += 32*5
						...
						for curItem in MONSTER_INFO_DATA[race]["items"]:
							...
							if -1 != emptySlotPos:
								...
								if itemSize[1] == 2:
									self.itemArray[emptySlotPos + self.X_SLOT] = 1
								elif itemSize[1] == 3:
									self.itemArray[emptySlotPos + self.X_SLOT] = 1
									self.itemArray[emptySlotPos + self.X_SLOT*2] = 1
